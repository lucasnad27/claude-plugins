...
**Produce a concrete reference artifact wherever the work has a shape you can render.** A self-contained HTML mockup settles more design questions than three paragraphs describing the same screen, and real JSON settles an API contract faster than a description of it. Write mockups alongside the design doc as `.rpi/design-YYYY-MM-DD-description.html` and link them from the Concrete Reference section. If an earlier HTML view (a `/show-me` page) settled a question, move it there too; later phases read only what the design links. The user reviews the artifact, not your description of it — and the implementing agent builds against it directly.
...
2. When options differ in shape, put each option's tree sketch in its `preview`. Previews are single-select only and render as monospace, so use text trees, not Mermaid.
